...
export class AlexaAuth {
  ...
  async getAuthenticatedClient(): Promise<AlexaRemote> {
    const alexaConfig = this.config.sources.alexa;
    ...
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        proxyPort: alexaConfig.proxy_port || 3001,
        proxyListenBind: '127.0.0.1',
        amazonPage: alexaConfig.amazon_page || 'amazon.com',
      ...
      if (!savedCookie?.cookie) {
        ...
        console.log(`http://localhost:${alexaConfig.proxy_port || 3001}`);
  ...
  private loadCookie(): AlexaCookie | null {
    const cookiePath = this.config.sources.alexa.cookie_path;
